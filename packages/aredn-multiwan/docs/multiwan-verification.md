# PollyWAN r12 build and verification
...
make -C openwrt package/aredn-multiwan/compile V=sc -j1 2>&1 | tee /tmp/pollywan-r12-build.log
find openwrt/bin -name 'aredn-multiwan-0.1.0-r12.apk' -print -exec sha256sum {} \;
```
...
```sh
apk add --allow-untrusted /tmp/aredn-multiwan-0.1.0-r12.apk

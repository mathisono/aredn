...
- `wan3` is permanently the phone USB RNDIS/CDC input.
...
`wan3` is the phone-to-hAP USB RNDIS/CDC network. In PdaNet proxy mode, public IPv4 TCP must be converted to HTTP CONNECT by redsocks on the hAP.
